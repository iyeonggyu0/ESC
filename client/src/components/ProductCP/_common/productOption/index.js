...
const ProductOption = ({ textFun, setProductOption, productName, productOption }) => {
  ...
  const createSaveHandler = () => {
    if (editKey !== null || mod === 'Edit') {
      // 배열의 불변성을 유지하면서 index가 1인 객체의 값을 수정
      setProductOption((prevProductOption) => {
        const updatedOption = { ...prevProductOption[1] }; // index가 1인 객체 복사
        updatedOption.optionName = optionName; // 값을 수정
        updatedOption.ProductOptionProperty = attributeArr; // 값을 수정
        return [
          ...prevProductOption.slice(0, 1), // 수정된 객체를 포함하기 전까지의 객체들 복사
          updatedOption, // 수정된 객체 추가
          ...prevProductOption.slice(2), // 수정된 객체를 포함한 이후의 객체들 복사
        ];
      });
    } else {
      textFun(
